...
class DWTInverse3d_Laplacian(nn.Module):
    ...
    def __init__(self, J=1, wave='db1', mode='zero'):
        ...
        self.g0_dep = nn.Parameter(filts[0], requires_grad=True)
        self.g1_dep = nn.Parameter(filts[1], requires_grad=True)
        self.g0_col = nn.Parameter(filts[2], requires_grad=True)
        self.g1_col = nn.Parameter(filts[3], requires_grad=True)
        self.g0_row = nn.Parameter(filts[4], requires_grad=True)
        self.g1_row = nn.Parameter(filts[5], requires_grad=True)
...
class DWTForward3d_Laplacian(nn.Module):
    ...
    def __init__(self, J=1, wave='db1', mode='zero'):
        ...
        self.h0_dep = nn.Parameter(filts[0], requires_grad=True)
        self.h1_dep = nn.Parameter(filts[1], requires_grad=True)
        self.h0_col = nn.Parameter(filts[2], requires_grad=True)
        self.h1_col = nn.Parameter(filts[3], requires_grad=True)
        self.h0_row = nn.Parameter(filts[4], requires_grad=True)
        self.h1_row = nn.Parameter(filts[5], requires_grad=True)
    ...
    def forward(self, x):
        ...
        for j in range(self.J):
            ...
            b = SFB3D_Laplacian()
            reversed_ll = b(ll_new, self.g0_dep, self.g0_col, self.g0_row, mode)
...
class DWT3d_Laplacian(nn.Module):
    """ Performs a 2d DWT Forward decomposition of an image
    Args:
        J (int): Number of levels of decomposition
        wave (str or pywt.Wavelet or tuple(ndarray)): Which wavelet to use.
...
class DWT3d_Laplacian_1(nn.Module):
    """  encoder
    Performs a 2d DWT Forward decomposition of an image
    Args:
        J (int): Number of levels of decomposition
...
class DWT3d_Laplacian_2(nn.Module):
    """ decoder
      Performs a 2d DWT Forward decomposition of an image 
    Args:
        J (int): Number of levels of decomposition
...
class SFB3D_Laplacian(nn.Module):
    ...
    """
    # @staticmethod
    def forward(self, low, g0_dep, g0_col, g0_row, mode):
...
class AFB3D_Laplacian(nn.Module):
    """ Does a single level 2d wavelet decomposition of an input. Does separate
    row and column filtering by two calls to
    :py:func:`pytorch_wavelets.dwt.lowlevel.afb1d`
    Needs to have the tensors in the right form. Because this function defines
    ...
    # @staticmethod
    def forward(self, x, h0_dep, h0_col, h0_row, mode):
