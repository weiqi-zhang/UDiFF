...
from models.module.dwt import DWTInverse3d_Laplacian, DWTForward3d_Laplacian, DWT3d_Laplacian
...
from utils.mesh import process
import time
...
diffusion_folder = r'<path to diffusion>' # put the category
epoch = 5000 # diffusion model epoch number
...
high_level_folder = r'<path to detail predictor>' # put the category
high_level_epoch = 1000 # detail predictor model epoch number
...
def one_generation_process(args):
    ...
    with torch.no_grad():
        ...
        dwt_3d_lap = DWT3d_Laplacian(J=config.max_depth, wave=config.wavelet, mode=config.padding_mode).to(device)
        checkpoint = torch.load('<path to ckpt>', map_location=device)
        dwt_3d_lap.load_state_dict(checkpoint['dwt__3d_lap'])
        ...
        for m in range(testing_cnt):
            ...
            voxels_pred = dwt_3d_lap.inverse((low_samples, highs_samples))
            udf = voxels_pred[0,0,:,:,:].cpu().detach().numpy()
            np.save(os.path.join(folder_path, f'{m+start_index}_{testing_sample_index}.npy'), udf)

            process(udf, os.path.join(folder_path, f'{m+start_index}_{testing_sample_index}_udf.obj'))
